...
def scan_buy_candidates(portfolio_value: float = 10000000, top_n: int = 20):
    ...
    try:
        ...
        for idx, row in buy_list.iterrows():
            ...
            print(f"  현재가:         {float(row['current_price']):>10,.0f}원")
            print(f"  종합 스코어:    {float(row['composite_score']):>10.1f}/100")
            print(f"  전략 타입:      {row['strategy_type']}")
            print(f"  추천 수량:      {int(row['recommended_shares']):>10,}주")
            print(f"  투자 금액:      {float(row['recommended_value']):>10,.0f}원 ({float(row['position_pct'])*100:.1f}%)")
            print(f"  손절가:         {float(row['stop_loss']):>10,.0f}원 ({(float(row['stop_loss'])/float(row['current_price'])-1)*100:.1f}%)")
            print(f"  목표가:         {float(row['profit_target']):>10,.0f}원 ({(float(row['profit_target'])/float(row['current_price'])-1)*100:.1f}%)")
            print(f"  손익비:         {float(row['risk_reward_ratio']):>10.2f}:1")
            print(f"  리스크 스코어:  {float(row['risk_score']):>10.1f}/100")
            print(f"  ATR:            {float(row['atr']):>10,.0f}원")
            print(f"  모멘텀:         {float(row['momentum_score']):>10.1f}%")
            print(f"  평균회귀:       {float(row['mean_reversion_score']):>10.1f}%")
            print(f"  전일 대비:      {float(row['d1_diff_rate']):>10.2f}%")
            print(f"  거래량 비율:    {float(row['volume_ratio']):>10.2f}x")
