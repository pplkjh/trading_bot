...
# ── 설정 ─────────────────────────────────────────────────────────
MIN_SCORE = {'A': 60, 'B': 50, '6': 50}   # 백테스트용 최저 임계값
...
def run_backtest(strategy):
    ...
    if strategy == '6':
        ...
        min_b = MIN_SCORE['B']
        print(f"\n{'='*65}")
        print(f"▶  Strategy 6 (A+B 혼합)  백테스트 시작")
        print(f"   min_score_a={min_a}  min_score_b={min_b}")
...
def _recommend(cumul_results, strategy_label):
    ...
    print(f"\n  💡 권장 min_score: {recommended}  → cf.py: v4_min_score_{strategy_label} = {recommended}")
