ver = "#version 2.0.0 - Advanced Strategy"
print(f"trader_advanced Version: {ver}")

from library.open_api import *
from PyQt5.QtWidgets import *

logger.debug("trader_advanced start !!!!!!")
...
class TraderAdvanced(QMainWindow):
    ...
    기존 trader.py + 고급 전략 시스템 통합
    """

    def __init__(self):
        logger.debug("TraderAdvanced __init__!!!")
        super().__init__()
        # openapi 초기화
    ...
    def variable_setting(self):
        ...
        logger.info(f"고급 전략 설정:")
        logger.info(f"  - 고급 매수 전략: {'사용' if self.use_advanced_strategy else '미사용'}")
        logger.info(f"  - 날짜 기반 전략: {'전용' if self.force_date_strategy else '혼합'}")
        logger.info(f"  - 고급 매도 전략: {'사용' if self.use_advanced_exit else '미사용'}")
    ...
    def get_sell_list_trade(self):
        ...
        if self.use_advanced_exit:
            ...
            self.sell_list_advanced = self.open_api.get_advanced_sell_list()

            # 고급 매도 리스트를 기존 형식으로 변환
            self.sell_list = []
            for signal in self.sell_list_advanced:
                if signal['decision']['should_exit']:
                    # (code, rate, present_price, valuation_profit) 형식으로 변환
                    self.sell_list.append([
                        signal['code'],
                        signal.get('pnl_pct', 0),
                        signal.get('current_price', 0),
                        0  # valuation_profit는 사용하지 않음
                    ])

            logger.info(f"고급 청산 전략: {len(self.sell_list)}개 매도 시그널")
        else:
            # 기존 방식
            logger.info("📉 기존 방식으로 매도 리스트 생성")
    ...
    def market_time_check(self):
        ...
        if self.current_time > self.market_start_time and self.current_time < self.market_end_time:
            logger.debug('''장시간입니다. 테스트하실 경우는 trader_advanced.py 파일의 variable_setting 함수에서
                     ### 모의, 실전 ### 부분을 주석 처리하시고
                     ### 테스트용 ### 부분의 주석을 풀어주시기 바랍니다.
                     실제로 모의/실전 투자 시 반드시 variable_setting을 원복해 주셔야 합니다.  ''')
            return True
        else:
            logger.debug('''장시간이 아닙니다.테스트하실 경우는 trader_advanced.py 파일의 variable_setting 함수에서
                     ### 모의, 실전 ### 부분을 주석 처리하시고
                     ### 테스트용 ### 부분의 주석을 풀어주시기 바랍니다.
                     실제로 모의/실전 투자 시 반드시 variable_setting을 원복해 주셔야 합니다. ''')
            return False
